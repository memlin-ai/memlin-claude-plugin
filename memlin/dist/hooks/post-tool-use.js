...
function repoRelativePath(absPath, cwd) {
  try {
    ...
    if (top) {
      const rel = path9.relative(top, absPath);
...
${planText}`;
  try {
    await fs6.writeFile(abs, fileBody.endsWith("\n") ? fileBody : fileBody + "\n", "utf8");
  } catch (err) {
...
async function main() {
  ...
  const plansDir = path11.join(os8.homedir(), ".claude", "plans");
  if (!abs.startsWith(plansDir + path11.sep)) return;
  ...
  try {
    ...
    log(
      `pushed plan ${path11.basename(abs)} (${result.created ? "new" : "v" + result.version_number}, project ${resolved.project_id?.slice(0, 8) ?? "(none)"})`
    );
  } catch (err) {
