...
${planText}`;
  try {
    await fs8.writeFile(abs, fileBody.endsWith("\n") ? fileBody : fileBody + "\n", "utf8");
  } catch (err) {
...
async function main() {
  ...
  if (!file) return;
  const abs = path18.resolve(file);
  const plansDir = path18.join(os12.homedir(), ".claude", "plans");
  if (!abs.startsWith(plansDir + path18.sep)) return;
  ...
  try {
    ...
    gitRemote = normalizeGitRemote(url);
  } catch {
  }
  try {
    const result = await pushPlanFile(ctx.api, abs, {
    ...
    log(
      `pushed plan ${path18.basename(abs)} (${result.created ? "new" : "v" + result.version_number}, project ${resolved.project_id?.slice(0, 8) ?? "(none)"})`
    );
  } catch (err) {
