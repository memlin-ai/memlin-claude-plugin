...
function runtimeCwd(fallback = process.cwd()) {
  for (const name of WORKSPACE_ENV_VARS) {
    ...
    if (raw && path2.isAbsolute(raw)) return path2.resolve(raw);
  }
  return path2.resolve(fallback);
}
...
function detectGitRemotes(cwd) {
  ...
  try {
    ...
    for (const entry of readdirSync(cwd, { withFileTypes: true })) {
      ...
      scanned++;
      const child = path2.join(cwd, entry.name);
      if (!existsSync(path2.join(child, ".git"))) continue;
